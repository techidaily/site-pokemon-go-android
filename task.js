...
const publishTask = () => {
  ...
  let error = null;
  const criticalError = `fatal: 无法访问 'https://github.com/`;
  ...
  const checkError = (e) => {
    ...
    if (e instanceof Error) {
      try {
        ...
        if (e.toString().indexOf(criticalError) > -1) {
          error = e;
        }
      } catch (err) { }

      try {
        if (e.message.indexOf(criticalError) > -1) {
          error = e;
        }
      } catch (err) { }
    }
